...
public class Client
{
  ...
  private String processLeaveRequest(StringTokenizer st)
  {
  ...
  private String processSearchRequest(StringTokenizer st)
  {
    final String ip = st.nextToken();
    final String port = st.nextToken();
  ...
  private String search(final Query query, int hops, Set<Node> propagatedNodes)
  {
    ...
    String[] queryWords = query.getQuery().split("_");
    List<String> foundFiles = new ArrayList<>();
    fileNames.forEach(fileName -> {
      String[] fileNameWords = fileName.split("_");
  ...
  private String propagateSearch(Query query, int hops, Set<Node> propagatedNodes)
  {
    try (DatagramSocket socket = new DatagramSocket())
    {
      for (Node node : connectedNodes)
      {
        ...
        String propagatedNodesString = convertPropagatedNodesToString(propagatedNodes);
  ...
  private String convertPropagatedNodesToString(Set<Node> nodeCollection)
  {
    StringBuilder propagatedNodesBuilder = new StringBuilder(nodeCollection.size());
  ...
  private void joinBS()
  {
    try (DatagramSocket socket = new DatagramSocket())
    {
      ...
      String unRegResponse = util.sendMessage(unregisterMessage.getBytes(), bsServer.getIp(), socket, port, Util.DEFAULT_TIMEOUT);
    ...
    String response = util.sendMessage(joinMessage.getBytes(), bsServer.getIp(), socket, port, Util.DEFAULT_TIMEOUT);
...
//    need to send hash of file names
    ...
    String response = util.sendMessage(buf, node.getIp(), socket, node.getPort(), Util.DEFAULT_TIMEOUT);
